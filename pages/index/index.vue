...
<template>
	<view class="page_box">
		...
		<view v-else class="page_box shopro-selector">
			...
			<view class="content_box" style="margin-top: -4rpx;overflow: hidden;">
				<scroll-view class="scroll-box" scroll-y scroll-with-animation enable-back-to-top>
					<block v-if="template" v-for="(item, index) in template" :key="index">
						<!-- 搜索 -->
						<sh-search v-if="item.type === 'search'" :detail="item.content" :bgcolor="bgcolor"></sh-search>
						<!-- 轮播 -->
						<sh-banner v-if="item.type === 'banner'" :detail="item.content" :bgcolor="bgcolor" @getbgcolor="getbgcolor">
						</sh-banner>
						<!-- 菜单 -->
						<sh-menu v-if="item.type === 'menu'" :detail="item.content" :menu="item.content.style"
							:imgW="94"></sh-menu>
						<!-- 推荐商品  old-->
						<sh-hot-goods v-if="item.type === 'goods-list' || item.type === 'goods-group'"
							:detail="item.content" :decorStyle="item.content.style"></sh-hot-goods>
						<!-- 广告魔方 -->
			...
			<!-- 小程序过期弹窗 -->
			<!-- #ifdef MP-WEIXIN -->
			<shopro-skip v-show="appcfgdata.is_jump == 1" :site="2"></shopro-skip>
			<!-- #endif -->
			<!-- 连续弹窗提醒 -->
						...
						<view class="service-contract-wrap">
							...
							<view class="service-content ">
								...
								<text style="color: #EAB866;"
									@tap="jump('/pages/public/richtext', { id: appcfgdata.store.privacy_agreement })">《商城隐私协议》</text>
	...
	export default {
		...
		data() {
			return {
				...
				noNetworkData: {
					img: this.$IMG_URL + '/imgs/empty/empty_network.png',
					tip: '网络连接断开,请刷新重试~',
					pathText: '刷新重试',
					type: 'netWork'
				},
		...
		onLoad(options) {
			...
			// #endif
			// 首次进入先加载启动图
			var isPlayd = uni.getStorageSync('launchFlag');
			if (!isPlayd) {
				uni.redirectTo({
