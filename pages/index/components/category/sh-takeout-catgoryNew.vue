				...
				<scroll-view scroll-y class="scroll-box righ-scroll-box flex-sub" :scroll-top="scrollRightTop" @scrolltolower="lowerMore" scroll-with-animation @scroll="rightScroll">
					<view class="right" v-if="categoryData.length">
						<view class="item-list" >
							...
							<view class="item-box">
								<view class="goods-item x-f mb30"  v-for="(mlist, index2) in categoryData" :key="index2">
									<view @tap="jump('/pages/goods/detail/index', { id: mlist.id })" class="item-img" style="overflow: hidden;">
										<!-- <image class="item-img" lazy-load :src="mlist.image" mode="aspectFill"></image> -->
										<u-lazy-load class="item-img" :image="mlist.image" :loading-img="loadingTmg" :error-img="errorImg" img-mode="widthFix"></u-lazy-load>
										<image v-if="mlist.activity" class="tag-img" :src="tagPath[mlist.activity.type]" mode=""></image>
									</view>
									<view class="goods-item--right" style="width: 350rpx;">
		...
		getCategory(nowId) {
			this.$api('category.goodsNew', {
				...
				if (res.code === 1) {
					...
					// this.catTopImg = res.data.goods.categorie.image;
					// this.catTopTit = res.data.goods.categorie.name;
					
					// this.categoryData = res.data.goods.data;
					// this.lastPage = res.data.goods.last_page
