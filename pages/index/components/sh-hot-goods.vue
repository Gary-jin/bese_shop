<template>
	...
	<view class="hot-goods mx20 mb10" v-if="goodsList.length">
		<!-- 一行  左图右字   -->
		<view v-if="decorStyle == 1">
			<view class="goods-item oneitembt" v-if="goods.id" v-for="goods in goodsList" :key="goods.id">
				<shopro-goods-onerow :detail="goods" :isTag="true"></shopro-goods-onerow>
			</view>
		</view>
		<!-- 一行 上图下字 -->
		<view v-if="decorStyle == 2">
			<view class="goods-item oneitembt" v-if="goods.id" v-for="goods in goodsList" :key="goods.id">
				<shopro-goods-onerowtb :detail="goods" :isTag="true"></shopro-goods-onerowtb>
			</view>
		</view>
		<!-- 两行 没有瀑布流 -->
		<view class="goods-list x-f" v-if="decorStyle == 3">
		...
		<!-- 两行带瀑布流 -->
		<shopro-goods-flow v-if="decorStyle == 4" :list="goodsList" :imageSrcKey="'image'"></shopro-goods-flow>
...
<script>
	...
	export default {
		...
		props: {
			...
			decorStyle:{ //1= 左图右字  2=上图下字  3=两列  4=两列瀑布流
				type:Number,
				default: 3
			}
		},
