	...
	<view class="wrap-box">
		<view class="user-box">
			<block v-if="template.length" v-for="(item, index) in template" :key="index">
				...
				<!-- 推荐商品 -->
				<sh-hot-goods v-if="item.type === 'goods-list' || item.type === 'goods-group'" :detail="item.content" :decorStyle="item.content.style"></sh-hot-goods>
				<!-- 广告魔方 -->
			...
			<!-- <navigator url="plugin-private://wx34345ae5855f892d/pages/productDetail/productDetail?productId=27039655"><button type="default">小店ductId27039655</button></navigator>
			<button @click="xxx">推客商品</button>
			<button @click="shopBag">小商店购物袋</button>
			<button @click="shopOrder">小商店订单list</button>
			<button @click="shopGoodsDetail">小商店订单详情</button>
			<button type="default" @tap="jump('/pages/activity/marketing/scratchCard')">刮奖</button> -->
			<!-- 版本号 -->
			<view class="foot_box">
...
export default {
	...
	methods: {
		...
		// ...mapActions(['getUserInfo']),  //优化getOrderNum重复加载
		cc(){
			uni.navigateTo({
				url: "plugin-private://wx34345ae5855f892d/pages/productDetail/productDetail?productId=2155138"
			})
		},
		// 初始化
		init() {
		...
		xxx(){
			wx.navigateTo({
				url: `plugin-private://wx34345ae5855f892d/pages/productDetail/productDetail?productId=23803363&ticket=dGlja2V0NTg5NjUzMDI3MjE5NTM0NTcxMjk3MDA0Mg`,
			});
			
			// let appid  = 'wx664e5ea08104f6fb';//云翔四海服饰
			// let pathUrl= 'plugin-private://wx34345ae5855f892d/pages/productDetail/productDetail?productId=23803363&ticket=dGlja2V0NTg5NjUzMDI3MjE5NTM0NTcxMjk3MDA0Mg'
		...
		shopOrder(){
			//跳转到订单页，激活“待付款”标签
			const tabId = 'pendingPay';   //待付款 pendingPay  待收货 pendingRecevied   退款/售后 afterSale
			wx.navigateTo({
				url: `plugin-private://wx34345ae5855f892d/pages/orderList/orderList?tabId=${tabId}`,
			});
		},
		shopGoodsDetail(){
			let orderId = '1704287613863340'
			// let orderId = '1704013666445340'
			wx.navigateTo({
