...
<template>
	<view class="box">
			...
			<view class="detail-content">
				<view class="goodes_detail_swiper-box">
					...
					<swiper class="carousel" circular @change="swiperChange" :autoplay="true">
						<swiper-item @tap="tools.previewImage(goodsInfo.images, swiperCurrent)"
							v-for="(img, index) in goodsInfo.images" :key="index" class="carousel-item">
							<image class="swiper-image shopro-selector-rect" :src="img" mode="aspectFill" lazy-load>
							</image>
						</swiper-item>
					</swiper>
					<view v-if="goodsInfo.images" class="swiper-dots">{{ swiperCurrent + 1 }} /
						{{ goodsInfo.images.length }}</view>
				</view>
				...
				<shopro-sku v-model="showSku" :goodsInfo="goodsInfo"
				...
				<!-- 优惠券 -->
				<view v-if="goodsInfo.is_share == 0">
					<sh-coupon
				...
				<!-- 拼团玩法 -->
				<view
					v-if="goodsInfo.activity && goodsInfo.activity.richtext_id != 0 && goodsInfo.activity_type !== 'seckill'"
					class="groupon-play x-bc px20"
					@tap="jump('/pages/public/richtext', { id: goodsInfo.activity.richtext_id })">
					<view class="x-f">
						<text class="title">玩法</text>
						<view class="description one-t">
							{{ goodsInfo.activity.richtext_title}}</view>
					</view>
					<text class="cuIcon-right" style="color: #bfbfbf;"></text>
				</view>
				<!-- 选项卡 -->
				<view class="sticky-box">
					<view class="tab-box x-f" :style="'opacity:' + opacityNum">
						<view class="tab-item y-f x-c" @tap="sTp>10 && onTab(tab.id)" v-for="tab in tabList" :key="tab.id">
							<view class="tab-title" :class="{ 'sizeCol': tabCurrent === tab.id }">
								{{ tab.title }}
					...
					<view>
						<view class="jifenTit">
							...
							<text class="jifenTxt" v-if="tabCurrent === 'tab0'">商品详情</text>
							<text class="jifenTxt" v-if="tabCurrent === 'tab1'">规格参数</text>
							<text class="jifenTxt" v-if="tabCurrent === 'tab2'">用户评价</text>
						</view>
					</view>
					<view class="tab-detail pb20">
						<view class="rich-box" v-show="tabCurrent === 'tab0'">
							<u-parse :tag-style="tagstyle" v-if="goodsInfo.content" :lazy-load="true" :html="goodsInfo.content"></u-parse>
							<view class="empty-box x-c" style="padding-top: 30rpx;" v-else>
						...
						<view class="goods-size" v-if="tabCurrent === 'tab1'">
							<view class="table-box" v-if="goodsInfo.params && goodsInfo.params.length">
								<view class="t-tr x-f" v-for="t in goodsInfo.params" :key="t.title">
									<view class="t-head one-t">{{ t.title }}</view>
									<view class="t-detail one-t">{{ t.content }}</view>
								</view>
							</view>
						...
						<view class="goods-comment" v-if="tabCurrent === 'tab2'">
							...
							<view class="more-box x-c" v-if="commentNum > 3">
								<button class="cu-btn more-btn x-f"
									@tap="jump('/pages/goods/comment-list', { goodsId: goodsInfo.id })">
									查看全部
									<text class="cuIcon-right"></text>
								</button>
							</view>
		...
		<!-- 积分商品foot -->
		<view class="score-foot-box x-ac" v-if=" detailType === 'score'">
			<view class="left x-f">
				<view class="tools-item y-f" @tap="goHome">
					<image class="tool-img" :src="$IMG_URL + '/imgs/tabbar/tab_home_sel.png'" mode="widthFix">
					</image>
		...
		<!-- 其他商品foot -->
		<view class="detail-foot_box  x-ac" v-if=" detailType !== 'score'">
			<view class="left x-ac">
				<view class="tools-item y-f" @tap="goHome">
					<image class="tool-img shopro-selector-circular"
						:src="$IMG_URL + '/imgs/tabbar/tab_home_sel.png'" mode="widthFix"></image>
				...
				<view class="tools-item y-f" @tap="onFavorite(goodsInfo.id)">
					<image class="tool-img"
						:src="Boolean(goodsInfo.favorite) ? $IMG_URL + '/imgs/favorite_end.png' : $IMG_URL + '/imgs/favorite.png'"
						mode="widthFix"></image>
					<text class="tool-title">收藏</text>
				</view>
				<view class="tools-item y-f" @tap="onShare">
					<image class="tool-img" :src="$IMG_URL + '/imgs/share.png'" mode="widthFix"></image>
			...
			<view class="detail-right">
				...
				<view class="detail-btn-box x-ac"
					...
					<button class="cu-btn  seckill-btn" v-if="activityRules.status === 'ing'"
						@tap="goSeckill">立即秒杀</button>
				...
				<view class="detail-btn-box x-ac"
					...
					<view class="x-f" v-if="activityRules.status == 'ing'">
						<button class="cu-btn tool-btn add-btn y-f" @tap="payGroupon"
							v-if="goodsInfo.activity.rules.is_alone === '1'">
							<text class="price">￥{{ goodsInfo.price }}</text>
							<text class="price-title">单独购买</text>
						</button>
						<button v-if="goodsInfo.groupon_disable == 0" class="cu-btn tool-btn groupon-btn y-f" 
						...
						<button v-if="goodsInfo.groupon_disable == 1" class="cu-btn tool-btn add-btn y-f"
							:style="goodsInfo.activity.rules.is_alone === '0' ? 'width:400rpx' : ''"
							@tap="$u.toast('团队已存在，请去参团！')">
							<text class="price">￥{{ goodsInfo.groupon_price }}</text>
							<text class="price-title">我要开团</text>
						</button>
					</view>
	...
	export default {
		...
		data() {
			return {
				...
				// tagstyle :{
				// 	div :'width:750rpx',
				// 	a :'width:750rpx !important;',
				// 	img: 'max-width:750rpx;min-width:750rpx;width:750rpx',
				// 	trees:'padding: 0;width: 750rpx;',
				// 	ul:'padding: 0;width:750rpx;',
				// 	area:'width: 750rpx;',
				// 	table:'margin:0 0 0 0 !important'
				// },
				tagstyle: {
					img: 'max-width:750rpx;min-width:750rpx;width:750rpx'
				},
		...
		computed: {},
		onUnload() {
			uni.$emit('delGrouponTimer')
		},
		onPageScroll(e){
			let scrollTop = e.scrollTop
			this.sTp = scrollTop
			if(scrollTop == 0) { this.opacityNum = 0 }; 
			if(scrollTop > 0 && scrollTop < 40) { this.opacityNum = 0.1 };
			if(scrollTop > 40 && scrollTop < 80) { this.opacityNum = 0.2 };
			if(scrollTop > 80 && scrollTop < 120) { this.opacityNum = 0.3 };
			if(scrollTop > 120 && scrollTop < 160) { this.opacityNum = 0.4 };
			if(scrollTop > 160 && scrollTop < 200) { this.opacityNum = 0.5 };
			if(scrollTop > 200 && scrollTop < 240) { this.opacityNum = 0.6 };
			if(scrollTop > 240 && scrollTop < 280) { this.opacityNum = 0.7 };
			if(scrollTop > 280 && scrollTop < 320) { this.opacityNum = 0.8 };
			if(scrollTop > 320 && scrollTop < 360) { this.opacityNum = 0.9 };
			if(scrollTop > 360 ) { this.opacityNum = 1 }; 
		},
		onLoad() {
		...
		methods: {
			...
			onTab(id) {
				this.tabCurrent = id;
				if (this.goodsInfo.coupons && this.goodsInfo.coupons.length && 
				this.goodsInfo.activity_type !== 'seckill' && this.goodsInfo.activity_type !== 'groupon' 
				&& this.detailType !== 'score'){
					uni.pageScrollTo({
						scrollTop: 300,
						duration: 100
					});
...
<style lang="scss" scoped>
	[is="uview-ui/components/u-parse/u-parse"] {
		overflow: hidden;
	}
