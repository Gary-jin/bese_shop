<template>
	<view class="group-people shopro-selector-rect">
		<block v-if="grouponTeamList.length">
			<view class="into-title x-bc" @tap="onMoreGrouponTeam">
			...
			<view class="into-item x-bc" v-for="(g, index) in grouponTeamList" :key="g.id" v-if="index < 2">
				...
				<view class="x-f">
					<view class="y-end">
						...
						<view class="time" v-if="g.expiretime && g.time && g.status == 'ing'">剩余时间{{ g.time.h }}:{{ g.time.m }}:{{ g.time.s }}</view>
					</view>
					<button class="cu-btn join-btn" v-if="g.status == 'ing'" @tap="jump('/pages/activity/groupon/detail', { id: g.id })">去参团</button>
					<button class="cu-btn join-btn-none" v-else >{{g.status == 'invalid'?'已过期':'已结束'}}</button>
				</view>
			</view>
		</block>
		<!-- 弹窗 -->
					...
					<view class="modal-content content_box y-f">
						<view class="into-item x-bc" v-for="g in grouponTeamList" :key="g.id">
							...
							<view class="x-f">
								...
								<button class="cu-btn join-btn" v-if="g.status == 'ing'" @tap="joinTeam('/pages/activity/groupon/detail', { id: g.id })">去参团</button>
								<button class="cu-btn join-btn-none" v-else >{{g.status == 'invalid'?'已过期':'已结束'}}</button>
							</view>
						</view>
					</view>
...
export default {
	...
	created() {
		...
		uni.$on('delGrouponTimer',function(data){
