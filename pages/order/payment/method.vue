...
<template>
	<view class="page_box">
		...
		<view class="content_box">
			...
			<radio-group @change="selPay" class="pay-box" v-if="payment">
				...
				<label class="x-bc pay-item" v-if="payment.includes('alipay')">
					...
					<radio value="alipay" :class="{ checked: payType === 'alipay' }" class="pay-radio orange"
				...
				<label class="x-bc pay-item" v-if="payment.includes('iospay') && !isAndroid">
					...
					<radio value="iospay" :class="{ checked: payType === 'iospay' }" class="pay-radio orange"
				...
				<label class="x-bc pay-item" v-if="payment.includes('wallet')">
					...
					<radio value="wallet" :class="{ checked: payType === 'wallet' }" class="pay-radio orange"
		...
		<shopro-pay ref="jpPwds" @completed="completed" @inputPwd="inputPwd" @forget="forget" :money="orderDetail.total_fee"
...
<script>
	...
	import ShoproPaytrueque from '@/common/shopro-pay-trueque';
	import {
		mapMutations,
		mapActions,
		mapState
	...
	export default {
		...
		methods: {
			completed(e) {
				this.passer = e
				let pay = ''
				if(this.options.type=='trueque'){
					pay = new ShoproPaytrueque(this.payType, this.orderDetail,this.passer,this.options.type);
				} else{
					pay = new ShoproPay(this.payType, this.orderDetail,this.passer);
				}
			...
			confirmPay() {
				...
				if(that.options.type=='trueque'){
					if (this.payType === 'wallet') {
						...
						that.passer = ''
						let pay = new ShoproPaytrueque(that.payType, that.orderDetail,that.passer);
					}
				} else {
					if (this.payType === 'wallet') {
						this.pwd.isPwy = true
						...
						that.passer = ''
						let pay = new ShoproPay(that.payType, that.orderDetail,that.passer);
					}					
				}
			},
			// 支付信息
