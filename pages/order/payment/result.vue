	...
	<view class="success-page">
		<view class="success-box flex flex-direction align-center">
			...
			<view class="btn-box flex justify-between">
				...
				<button class="cu-btn base-btn mx10" v-if="trueque!='trueque'" @tap="onOrder">查看订单</button>
				<button class="cu-btn base-btn mx10" v-if="trueque=='trueque'" @tap="onOrder1">查看订单</button>
...
export default {
	...
	methods: {
		...
		onOrder1(){
			this.$Router.replace({
				path: '/pages/trueque/orderDetail',
				query: { id: this.orderDetail.id }
			});
		},
		// 支付信息
		...
		onPay() {
			let that = this;
			if(that.trueque == 'trueque'){
				new ShoproPaytrueque(that.$Route.query.type, that.orderDetail);
			} else {
				new ShoproPay(that.$Route.query.type, that.orderDetail);
			}
		}
	}
};
