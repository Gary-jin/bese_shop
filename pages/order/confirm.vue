...
<template>
	<view class="">
		...
		<view class="content_box">
			<!-- 确认订单商品卡片 -->
			<view class="coreBox1">
				<view class="goods-list" v-for="(g,index) in perGoodsList" :key="g.sku_price_id">
					...
					<view style="padding: 0 25rpx;">
						<view v-if="index == perGoodsList.length -1" class="logistic item-list x-bc" @tap="onSelExpressType(g)">
			...
			<view style="padding: 0 30rpx;margin-top: 20rpx;">
				<view style="border-radius: 18rpx;overflow: hidden;">
					<view class="remark-box coupon x-bc item-list" v-if="is_share ==0 &&!orderPre.activity_type && orderType !== 'score'" @tap="selCoupon">
						<view class="item-title item-title1">优惠券</view>
						<view class="x-f" >
							<text class="price1" v-if="pickerData.couponList.length">{{ pickerData.title }}</text>
							<text class="sel-coupon" v-else>暂无优惠券</text>
							<text class="cuIcon-right"></text>
						</view>
					</view>
					<view class="remark-box score item-list x-bc" v-if="orderType === 'score'">
						<view class="x-f"><text class="item-title item-title1">积分</text></view>
						<view class="price">-{{ orderPre.score_amount }}积分</view>
					</view>
					<view class="remark-box x-bc item-list border-top">
						<view class="item-title item-title1">商品金额</view>
						<view class="x-f">
					...
					<view class="totaBox">
						...
						<text class="totaPic">{{ orderPre.total_fee || '0.00' }}</text>
					</view>
				</view>
			</view>
			<!-- 备注 -->
			<view style="padding: 0 30rpx; margin-bottom: 140rpx;">				
				<view class="remark-box x-f beizhu">
					<view class="item-title item-title1">备注</view>
					...
					<view class="express-type__content content_box">
						...
						<view class="express-address" v-if="expressTypeCur == 'selfetch'">
							...
							<view class="" v-else>
								...
								<view class="express-bottom"  v-if="appcfgdata.store.store_service_agreement">   
									<label class="x-f" @tap="checkProtocol">
										...
										<view class="protocol">
											同意并接受 
											<text class="protocol-text" @tap.stop="jump('/pages/public/richtext', { id: appcfgdata.store.store_service_agreement })">《到店自提服务协议》</text>
										</view>
									</label>
								</view>
							</view>
		...
		<shopro-modal v-model="showCheckTime" :modalType="'bottom-modal'">
			<block slot="modalContent">
				<view class="checkTime-box page_box">
					<view class="checkTime-head" @tap="showCheckTime = false">
						选择{{ checkType }}时间
...
export default {
	...
	methods: {
		...
		// 更改提交数据
		changeGoodsList() {
			// 2021.3.17 new 到店自提  goodslist 都要加状态
			if (this.expressTypeCur == 'selfetch') {
