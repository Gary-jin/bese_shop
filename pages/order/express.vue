		...
		<view class="express-wrap">
			<view class="oilStation">
				<view class="oilStation-top">
					...
					<view class="express-sn x-f" @tap="copyCode(expressDetail.express_no)">
		...
		getExpressDetail() {
			...
			that.$api('order.expressDetail', {
				...
				if (res.code === 1) {
					...
					if(that.$Route.query.type == 'winlist'){
						let list = {
							goods_image: res.data.img,
							status_name:'已发货'
						}
						that.goodsList.push(list)
						that.firstGoods = that.goodsList[0]
					} else {
