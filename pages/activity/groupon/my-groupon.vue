	...
	<view class="page_box">
		<view class="use_head_box" style="background-color: #FFFFFF;">
			<cu-custom :isBack="true">
				<block slot="content" style="font-size: 34rpx;">我的拼团</block>
			</cu-custom>
		</view>
		<view class="head_box">
			<view class="order-nav x-f">
				<view class="nav-item y-f" v-for="nav in groupState" :key="nav.id" @tap="onNav(nav.id)">
					<view class="item-title" :class="{ 'titleCol': stateId === nav.id }">{{ nav.title }}</view>
			...
			<scroll-view scroll-y="true" enable-back-to-top @scrolltolower="loadMore" refresher-background="#f5f5f5" class="scroll-box">
				<block v-for="groupon in myGrouponList" :key="groupon.id">
					<view class="group-card">
						<image v-if="groupon.groupon.status=='finish'&&groupon.groupon.status=='finish-fictitious'" class="group-state" :src="grouponStatus[groupon.groupon.status]" mode=""></image>
