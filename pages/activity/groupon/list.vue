	...
	<view class="page_box">
		...
		<view class="content_box">
			<scroll-view class="scroll-box" scroll-y enable-back-to-top scroll-with-animation @scrolltolower="loadMore">
				<view class="group-wrap">
					...
					<view class="group-box">
						<view class="goods-item" v-for="(groupon, index) in grouponList" :key="groupon.id">
							<sh-activity-card :cardId="groupon.id" :title="groupon.title" :subtitle="groupon.subtitle"
								:img="groupon.image" :price="groupon.groupon_price"
								...
								<block slot="btn"><button class="cu-btn buy-btn"
										@tap.stop="jump('/pages/goods/detail/index', { id: groupon.id })">马上拼</button>
								</block>
							</sh-activity-card>
						</view>
					</view>
				...
				<!-- 加载更多 -->
				<u-loadmore v-if="grouponList.length" height="80rpx" :status="loadStatus" icon-type="flower"
					color="#ccc" />
				<!-- loading -->
