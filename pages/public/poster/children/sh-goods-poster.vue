<template>
	<view class="content">
		...
		<view class="hideCanvasView">
			<canvas class="hideCanvas" id="goods_poster" canvas-id="goods_poster" :style="{ width: (poster.width || 1) + 'px', height: (poster.height || 1) + 'px' }"></canvas>
		</view>
	</view>
</template>
		...
		let goodsInfo = that.getGoodsDetail().then(res => {
			if (that.shareInfo) {
				timer = setTimeout(function() {
					...
					console.log(that.shareInfo.path + "商品页面分享海报页面路径");
