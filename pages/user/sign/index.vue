        ...
        <view class="calendar">
          ...
          <view class="bar x-c">
            ...
            <view class="previous" @click="handleCalendar(0)">
              <button class="cu-btn bar-btn">
                <text class="cuIcon-back"></text>
              </button>
            </view>
            <view class="date"
              >{{ cur_year || "--" }} 年 {{ cur_month || "--" }} 月</view
            >
          ...
          <view class="myDateTable">
            <view v-for="(item, j) in days" :key="j" class="dateCell x-c">
              <view
                v-if="item.date == undefined || item.date == null"
                class="cell"
            ...
            <view class="x-c sign-box">
              <button
                :disabled="isPresentMonth || isSign"
                @tap="onSign"
                class="cu-btn sign-btn"
              >
                {{ isSign ? "已签到" : "签到" }}
              </button>
            </view>
      ...
      <block slot="modalContent">
        <view class="sign-modal-box">
          ...
          <view class="modal-bottom"
            ><button class="cu-btn confirem-btn" @tap="onConfirem">
              确认
            </button></view
          >
        </view>
...
export default {
  ...
  onLoad() {
    console.log(JSON.stringify(this.appcfgdata) + "vue配置信息");
  ...
  methods: {
    ...
    selSign() {
      let that = this;
      let selToday = `${that.toYear}-${that.toMonth
        .toString()
        .padStart(2, "0")}-${that.today.toString().padStart(2, "0")}`;
      let newDay = `${that.cur_year}-${that.cur_month
        .toString()
        .padStart(2, "0")}-${that.cur_day.toString().padStart(2, "0")}`;
      if (selToday === newDay) {
    ...
    calculateEmptyGrids() {
      ...
      const firstDayOfWeek = new Date(
        Date.UTC(that.cur_year, that.cur_month - 1, 1)
      ).getDay();
      if (firstDayOfWeek > 0) {
    ...
    onSign() {
      ...
      if (this.appcfgdata.score.sign_switch !== '1') {
        uni.showToast({
          title: '此功能暂未开放，敬请期待',
          icon: "none",
        });
      ...
      let month = that.cur_month.toString().padStart(2, "0");
      let query = `${that.cur_year}-${month}`;
      let today = `${that.cur_year}-${that.toMonth
        .toString()
        .padStart(2, "0")}-${that.today.toString().padStart(2, "0")}`;
      that
        .$api("user_sign.sign", {
    ...
    handleCalendar(type) {
      ...
      if (type === 0) {
        ...
        if (
          newYear < this.toYear ||
          (newYear === this.toYear && newMonth <= this.toMonth)
        ) {
...
.sign-modal-box {
  ...
  position: absolute;
  overflow: hidden;
  background: url($IMG_URL + "/imgs/singin.png") no-repeat;
