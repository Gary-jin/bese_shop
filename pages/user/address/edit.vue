...
<template>
	<view class="page_box" style="overflow-x: hidden;">
		...
		<view class="content_box coreBox">
			...
			<view class="address-item x-f">
				<label class="x-f flex-sub">
					...
					<input class="inp flex-sub" placeholder-style="color:#C1C1C1" placeholder="请填写收货人姓名" type="text" v-model="addressData.consignee" />
				</label>
			</view>
			<view class="address-item x-f">
				<label class="x-f flex-sub">
					<text class="item-title">手机号</text>
					<input class="inp flex-sub" placeholder-style="color:#C1C1C1" placeholder="请填写收货人手机号" type="number" v-model="addressData.phone" />
				</label>
			</view>
			<view class="address-item x-f" >
				<label class="x-f flex-sub">
					...
					<text class="cityBox more-t" @tap="selCity">{{area_text || '点击选择所在地区'}}</text>
					<view class="fillBox"  @tap="chooseLocation">
						<image class="fill" :src="$IMG_URL + '/imgs/order/fill.png'" mode=""></image>
						<text>定位</text>
					</view>
				</label>
			</view>
			<view class="area-box x-f">
				<label class="x-f x-start">
					...
					<textarea
						...
						class="area-inp"
						type="text"
						placeholder="请填写详细的地址信息，如果门牌号、小区、楼栋号、单元等"
			...
			<view class="default-box x-bc">
				...
				<switch class="editCol switch" type="switch"  @tap="onSwitch" :class="{ checked: addressData.is_default }" :checked="addressData.is_default"></switch>
			</view>
		</view>
