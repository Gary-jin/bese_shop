	...
	<view class="page_box">
		...
		<view class="content_box">
			<scroll-view scroll-y="true" @scrolltolower="loadMore" class="scroll-box">
				...
				<!-- 空白页 -->
				<shopro-empty v-if="emptyShow" :emptyData="emptyData"></shopro-empty>
				<!-- 更多 -->
...
export default {
	...
	methods: {
		...
		onTab(type) {
			...
			that.getScoreLog()
			// that.$u.debounce(that.getScoreLog);
		},
		// 积分明细
		getScoreLog(type) {
			let that = this;
			...
			that.$api('user_wallet_log', {
				...
				if (res.code === 1) {
					...
					if (that.currentPage === 1) that.scoreLog = []
					that.scoreLog = that.scoreLog.concat(arr)
