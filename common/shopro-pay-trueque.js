...
export default class ShoproPay {
	...
	constructor(payment, order,password) {
	...
	prepay() {
		...
		return new Promise((resolve, reject) => {
			...
			api('api.sCyPrepay', params).then(res => {
				if (res.code === 1) {
	...
	async wechatWapPay() {
		...
		if (result.code === 1) {
			...
			let params = encodeURIComponent(
				`${domain}/pages/order/payment/result?orderSn=${that.order.order_sn}&type=${that.payment}&trueque=trueque`)
