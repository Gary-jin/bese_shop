...
let _app = {
	...
	encryptPath(url) {
		let pathUrl = url
		let a = pathUrl.indexOf("com") + 3;
		let cc = pathUrl.indexOf("?");
		let b
		if (cc == -1) {
			b = pathUrl.length
		} else {
		...
		// path = pathUrl.substring(a, b)
		return pathUrl.substring(a, b)
		// console.log(path + "得到路径");
	},
	...
	downloadFile_PromiseFc(url) {
		return new Promise((rs, rj) => {
			if (url.substring(0, 4) !== 'http') {
				...
				log('url:' + url);
				console.log("url***",url)
				path = this.encryptPath(url)
				
				console.log(path + "得到路径");
				time = Date.parse(new Date()) / 1000
				randomNum = Math.random().toString(36).substr(2).toUpperCase();
				strone = `${appid}${path}${time}${token}${randomNum}${device}${version}`
				str = strone.toUpperCase()
				header.sign = hex_md5(hex_md5(str.toUpperCase()))
				header.time = time
				header.random = randomNum
				uni.downloadFile({
					url,
					header: header,
					success(res) {
