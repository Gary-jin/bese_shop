...
export default {
	...
	onLoad(options) {
		// console.log(JSON.stringify(JSON.stringify(options))+"分享")
		let that = this;
		//为每个页面设置分享信息
		that.setShareInfo();
		//直接进入页面
		...
		if (options.scene) {
			...
			let sanceINdex = scene.indexOf('share_id=')
			if(sanceINdex !== -1){
			 let hah =  scene.slice(sanceINdex+9)
			 options.share_id = hah
			}else{
		...
		// 2.保存推荐人信息   
		if (options.share_id) {
			// console.log(options.share_id + "分享信息")
		...
		// if(options.startOpportunity){
		// 	this.startOpportunity = options.startOpportunity
		// }
		if(options.webviewPath) {
			var url = decodeURIComponent(decodeURI(options.webviewPath));
			if(url.indexOf("startOpportunity")!= -1){
				this.startOpportunity = true
			}
		}

	},
	onUnload(){
		if(this.startOpportunity) {
			uni.$emit('backStart')
		}
	},
	methods: {
			...
			uni.getStorage({
				...
				success(e) {
					...
					if (userInfo && userInfo.id) {
						scene.query.share_id = userInfo.id//邀请码
					}
					//构造query参数链接
					that.shareInfo.path = ''
	...
	onShareAppMessage(res) {
		// console.log(JSON.stringify(res) + "分享")
		let that = this;
		return {
			...
			invite:1234,
			success(res) {
				console.log(JSON.stringify(res) + "分享后")
				uni.showToast({
					title: '分享成功'
				})
			},
